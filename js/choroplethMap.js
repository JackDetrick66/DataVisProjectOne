class ChoroplethMap {
  ...
  updateVis() {
    ...
    const popDensityExtent = d3.extent(vis.data.features, d => d.properties.lifeExp);
  ...
  renderVis() {
    ...
    const countryPath = vis.chart.selectAll('.country')
      ...
      .join('path')
        .attr('opacity', d=>getOpacity(d.properties.region))
        .attr('class', 'country')
        .attr('d', vis.geoPath)
        .attr('fill', d => {
          if (d.properties.lifeExp) {
            return vis.colorScale(d.properties.lifeExp);
          } else {
